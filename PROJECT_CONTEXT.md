...
- 마이클(#2) 히어로 배경은 Figma 원본 그대로 인물이 왼쪽에 오는 비반전 버전(`hero_bg_michael.png`)을 사용, 별도 `data-bg-flip` 없이 `center` 정렬(아래 참고 — 한때 좌우반전 버전을 썼다가 사용자 확인 후 원상복구함)
...
- **MoodCinema 슬라이드 전환을 스크롤 방식 → `is_active` 클래스 스왑 방식으로 전면 교체**: Figma는 4개 슬라이드가 가로로 나란히 배치된 정적 구조([1563:3118](index.html:299) `Scroll`, 6778.8px)지만, 실제로는 세 번의 반복 끝에("스크롤도 안되는데" → scrollLeft 누적 방식이 scroll-snap에 되튕김 / "mood_slide만 스크롤하면 바뀌도록 안에 컨텐츠만" → 컨테이너가 실제로 옆으로 밀리는 느낌 대신 콘텐츠만 바뀌길 원함) 최종적으로 **컨테이너(.mood_scroll)는 고정, `.mood_slide` 4개 중 `is_active` 클래스가 붙은 하나만 `display:flex`로 보이고 나머지는 `display:none`**인 구조로 재작성함(Special 탭 패널과 동일한 패턴, [styles.css:1029](styles.css:1029), [index.html:703](index.html:703)). 휠 이벤트로 `moodSlideIndex`를 증감시키며 `is_active`를 옮기고, 500ms 쿨다운으로 연속 휠 입력에 여러 장이 한번에 안 넘어가게 함. 첫/마지막 슬라이드 경계에서는 `preventDefault`를 호출하지 않아 페이지 세로 스크롤로 자연스럽게 이어짐. 컨테이너 자체가 더 이상 가로 스크롤되지 않으므로 스크롤바 숨김 처리는 불필요해져서 제거함. **이번 세션은 Figma/브라우저 MCP가 모두 연결 끊긴 상태라 실제 브라우저에서 시각 검증을 못 함 — 다음 확인 필요**
- **(위 재작성으로 대체됨) `.mood_slide` 모바일 반응형 죽은 코드 발견 및 수정**: 1280px/768px 미디어쿼리에서 `.mood_slide { grid-template-columns: 1fr }`로 되어 있었는데, `.mood_slide`는 애초에 `display:flex`라 `grid-template-columns`는 아무 효과가 없는 죽은 코드였음(그동안 진행바+텍스트+포스터가 모바일에서도 계속 가로 한 줄로 배치되고 있었을 가능성). `flex-direction:column`으로 교체해 실제로 세로 스택되도록 수정. 기존엔 `.mood_scroll`의 `overflow-x:auto`가 안전망 역할을 해서 페이지 레벨 가로 스크롤로 안 새어나왔지만, 이번에 그 안전망을 걷어냈으므로 `.mood_slide_posters`에 `max-width:100%`(768px 이하에서는 `max-width:320px`)도 추가로 넣어 모바일에서 진짜로 넘치지 않도록 보강([styles.css:1113](styles.css:1113), [styles.css:1648](styles.css:1648), [styles.css:1744](styles.css:1744))
- **본문 좌우 여백(`--page-padding-x`) 재보정**: 기존 `clamp(56px, 6vw, 160px)`은 Figma 기준 1920px 폭에서 약 115px로 계산되는데, 실제 Figma는 Hero/MoodCinema/Goods/Event 등 좌측 정렬 섹션 전부 220px 여백(콘텐츠 폭 1480px)을 씀. 사용자가 "사이즈 하나도 안 맞는다"고 지적한 근본 원인 중 하나로 파악 — 1920px에서 정확히 220px, 1280px 브레이크포인트에서 기존 40px로 자연스럽게 이어지는 `clamp(40px, 28.125vw - 320px, 220px)`로 교체([styles.css:129](styles.css:129)). 헤더는 Figma상 76px로 본문과 다른 값이라 `--header-padding-x: 76px`를 별도로 분리해 `.header_layout`에 적용
- **히어로 TOP5 위치 어긋남**: 위 여백 재보정이 근본 원인 — `--page-padding-x`가 실제보다 좁았던 만큼 `.hero_layout`의 `justify-content:space-between`으로 배치되는 TOP5 리스트도 Figma보다 안쪽(왼쪽)에 있었음. 위 여백 수정으로 함께 해결됨. 추가로 `.hero_top5` 폭도 516px→526px로 Figma 수치(526px)에 맞춤([styles.css:508](styles.css:508))
- **섹션별 배경색 불일치**: body/대부분 섹션은 `#141414`인데 `.hero_stage`와 `.lotte_pick_section`만 순수 `#000`을 하드엣지로 써서 섹션 경계에 색 띠가 보였음(`.mood_section`은 이미 gradient로 부드럽게 처리돼 있었음). Hero는 하단 오버레이 그라디언트의 마지막 stop을 `rgba(0,0,0,0.85)`→`#141414`로 바꿔 다음 섹션과 완전히 같은 색에서 만나도록([styles.css:301](styles.css:301)), LottePick은 `background:#000` 고정값을 `.mood_section`과 같은 방식의 `linear-gradient(180deg, rgba(20,20,20,0) 0%, #000 12%, #000 88%, rgba(20,20,20,0) 100%)`로 교체해 위아래 모두 부드럽게 빠지도록 수정([styles.css:768](styles.css:768)). **이 항목도 이번 세션엔 Figma 원본 배경색 토큰을 재확인하지 못한 채(Figma MCP 연결 끊김) 기존 코드 내 일관성 기준으로만 수정 — Figma 재연결 후 실제 배경색 값 대조 필요**
